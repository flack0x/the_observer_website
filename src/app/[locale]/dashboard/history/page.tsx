...
import { ArrowRight, ThumbsUp, History } from "lucide-react";
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-light flex items-center gap-2">
            <History className="h-6 w-6 text-earth-olive" />
            Interaction History
          </h1>
          <p className="text-slate-medium mt-1">Articles you've liked or reacted to.</p>
        </div>
      </div>
      ...
      <div className="grid gap-4">
        {likes && likes.length > 0 ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase ${
                      ...
                      {item.interaction_type === 'like' ? 'Liked' : 'Disliked'}
                    </span>
                    <span className="text-xs text-slate-dark">
                      {getRelativeTime(new Date(item.created_at), locale as any)}
                    </span>
                  </div>
          ...
          <div className="text-center py-20 bg-midnight-800 rounded-xl border border-midnight-700 border-dashed">
            <ThumbsUp className="h-12 w-12 text-slate-dark mx-auto mb-4" />
            <h3 className="text-lg font-bold text-slate-light mb-2">No history yet</h3>
            <p className="text-slate-medium mb-6">Interact with articles to see them here.</p>
            <Link 
