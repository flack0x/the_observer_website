...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: isArabic ? 'الملفات | المُراقِب' : 'The Dossier | The Observer',
    description: isArabic
      ? 'ملفات تعريفية للشخصيات الرئيسية في الجيوسياسة العالمية'
      : 'Profiles of key figures shaping global geopolitics',
  };
}
      ...
      <section className="border-b border-midnight-700 bg-midnight-800 py-16 sm:py-24">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-tactical-red/10 border border-tactical-red/20 mb-6">
            <Users className="h-4 w-4 text-tactical-red" />
          ...
          <p className="text-lg text-slate-medium leading-relaxed max-w-2xl mx-auto">
            {isArabic
              ? 'ملفات تعريفية شاملة للشخصيات الرئيسية - القادة العسكريين والسياسيين ورجال الاستخبارات الذين يشكلون الأحداث العالمية.'
              : 'Comprehensive profiles of key figures — military commanders, political leaders, and intelligence operatives shaping global events.'}
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
          <div className="bg-midnight-800 rounded-2xl border border-midnight-700 p-8 sm:p-12">
            ...
            <p className="text-slate-medium mb-8 max-w-md mx-auto">
              {isArabic
                ? 'نعمل على بناء قاعدة بيانات شاملة للشخصيات الرئيسية. تابعنا للتحديثات.'
                : 'We are building a comprehensive database of key figures. Follow us for updates.'}
            </p>
            <Link
